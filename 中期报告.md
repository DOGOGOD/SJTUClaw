...
目前项目已完成课程 Step 0 至 Step 9 的主要要求，并扩展了 Web UI、Windows 桌面应用、QQ Bot、桌面宠物、Reflection、Heartbeat、Pi 与 Claude Code 可选后端、microsandbox microVM 隔离、运行指标与安全加固。当前 Python 测试套件可收集 671 项测试，本轮基线为 `667 passed, 4 skipped`，另有 Vitest 前端测试覆盖主要交互；桌面程序已完成 PyInstaller 构建、Inno Setup 7 安装包生成和 Gateway 冒烟测试，sandbox 也已完成真实 microVM 冒烟验证。
...
| **Step 5：Tool 与 Agent Loop** | 已形成工具请求、执行、结果回写和继续推理闭环，覆盖文件、网页、Memory、Cron、Skill、Shell、附件和持久化临时下载。 |
| **Step 6：Gateway 与图形化入口** | 已实现 FastAPI Gateway、REST API、SSE 和 React Web UI；支持会话隔离附件上传、图片预览及普通文件下载按钮。 |
| **Step 7：Scheduler** | 支持一次性、固定间隔和 Cron 任务，并保存执行状态与历史。 |
| **Step 8：Workspace、Advanced Tool 与 Approval** | 支持 Workspace 绑定、文件写入、持久 Shell、附件复制、Approval 和可选 microVM 隔离。 |
...
- 运行路径隔离：源码版继续使用项目 `data/`，安装版将运行数据保存到 `%USERPROFILE%\.sjtuclaw\data`。
- QQ Bot 与桌面宠物：支持消息入口、工具审批、状态可视化和宠物资源管理。
- 可选外部 Agent 后端：Pi 通过 JSONL RPC、Claude Code 通过 `stream-json` 接入；二者保留各自原生工具循环和会话能力，并复用 SJTUClaw 的入口、Session、审批及宿主工具。
- microsandbox microVM：SJTUClaw 原生文件与 Shell 工具可按 Session 路由到隔离环境；通用依赖由基础镜像提供，项目依赖保存在 `/workspace/.venv`。
- Session 运行偏好：AUTO 与显式 Sandbox 开关随 Session 持久化，应用重启后恢复；UNLIMITED 作为临时越界权限始终在重启后关闭。fork 不继承权限状态，Sandbox 显式开启但运行环境不可用时拒绝回退到宿主执行。
- Web UI 输出文件交付：Agent 创建下载入口后，Gateway 自动补齐并去重消息中的下载 Markdown；图片显示预览与下载按钮，普通文件显示下载按钮，有效链接可跨 Gateway 重启恢复。
...
│   └── sandbox/        # microsandbox 路由、运行时与项目依赖同步
├── prompts/            # Prompt 资源
├── skills/             # 内置 Skill
├── webui/              # React + TypeScript 前端源码
├── web/                # 已构建前端静态文件
├── packaging/
│   ├── sandbox/        # microVM 基础镜像与构建脚本
│   └── windows/        # PyInstaller 与 Inno Setup 打包脚本
├── docs/               # 配置、测试与打包文档
├── tests/              # 自动化测试与 sandbox 冒烟测试
├── data/               # 源码运行时数据
...
2. **运行时层**：`agent/`、`context/`、`llm/` 负责原生推理循环；`pi/` 与 `claude/` 提供可选外部 Agent 路由和事件桥接。
3. **能力层**：`tools/`、`skills/`、`memory/`、`scheduler/` 提供环境操作、工作流、长期记忆和定时任务。
4. **安全与存储层**：`approval/`、`workspace/`、`sandbox/` 负责审批、路径边界与 microVM 隔离；Session、Memory、Cron 等运行数据使用文件系统持久化。
...
flowchart LR
    ...
    B --> C["Context Builder"]
    C --> D{"Session Agent 后端"}
    D -->|SJTUClaw| E["LLM Client + 原生 Agent Loop"]
    D -->|Pi / Claude Code| P["外部 Agent Runtime"]
    E --> F{"最终回复或 Tool Call"}
    P --> F
    F -->|最终回复| S["Session Store"]
    F -->|Tool Call| G["Tool Registry"]
    G --> H{"审批与 Sandbox 路由"}
    H -->|需审批| I["用户批准或拒绝"]
    H -->|宿主或 microVM| J["执行工具"]
    I --> J
    J --> S
    S --> C
```
...
| `Session` | `session_id`、`title`、`messages`、`summary`、`metadata` 等 | 表示独立会话，保存消息、摘要和运行偏好。 |
...
Compact 由三层机制构成：达到 token 阈值后由后台 worker 生成持久化摘要；用户可通过 `/compact` 手动压缩可安全归档的完整旧轮次；模型调用前修复孤立 Tool Result、补齐中断 Tool Call、截断超长结果并裁剪超预算历史。会话空闲本身不会触发压缩。Pi 优先使用原生 compact，Claude Code 自行管理原生上下文；System Prompt、Soul、Memory 和 Skill 索引作为稳定上下文，不会被普通 Session Compact 覆盖。

### 5.4 可扩展 Tool 与 Sandbox 系统
...
- 原生文件与 Shell 工具可路由到 microsandbox microVM，workspace 未设置或宿主路径越界时仍可在隔离工作区执行。
- 通用 Python 库固化进 `python:3.12-bookworm` 基础镜像，项目专属依赖同步到 `/workspace/.venv`，并随私有 volume 保留。
- 显式开启 Sandbox 后采用 fail-closed 策略，运行时不可用时不会静默回退到宿主。
...
Windows 桌面应用、Web UI、QQ Bot、CLI 和桌面宠物面向不同场景。桌面版复用 Gateway、Web UI 和统一 Agent Runtime；`desktop.py` 负责端口选择、服务启动和 pywebview 窗口创建，`paths.py` 区分源码、PyInstaller 资源和用户可写目录。安装版将运行数据保存到 `%USERPROFILE%\.sjtuclaw\data`，支持覆盖升级时保留用户数据。各入口只负责消息传输、进程启动和界面呈现。
...
4. **Sandbox 覆盖仍有限。** 当前 microVM 路由覆盖 SJTUClaw 原生文件与 Shell 工具，Pi、Claude Code 等外部 Agent 的原生工具尚未纳入同一隔离边界。
5. **渠道、测试和可观测性仍需补强。** 仍缺少更多消息渠道、浏览器级端到端测试、日志检索、费用统计和任务看板。
...
- **Sandbox**：扩充可复现基础镜像，完善网络、资源配额和外部 Agent 隔离策略。
...
目前 SJTUClaw 已经完成从“最小 LLM 调用”到“多入口、可记忆、可调用工具、可定时执行、受审批控制、可切换 Agent 后端、可选 microVM 隔离、可交付输出文件、可通过 Skill 扩展并可作为 Windows 软件安装”的演进。项目的主要成果不仅是实现功能，更是建立了统一 Runtime、分层 Context、长期 Memory、三层 Compact、Tool Registry、安全 Approval、Sandbox 路由和统一路径管理等可扩展架构。
