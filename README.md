# SJTUClaw — 面向个人与教学场景的 AI Agent Runtime

SJTUClaw 是一个可本地运行、可扩展、强调安全边界的 AI Agent 工作台。它把多轮对话、工具调用、长期记忆、Skill、定时任务和多种交互入口组合在一起，既适合学习 Agent Runtime 的实现，也适合搭建个人自动化助手。

项目后端使用 Python 实现，基于 FastAPI 提供 Gateway，兼容 OpenAI 风格的 Chat Completions 与 function calling；前端使用 React + TypeScript 构建，同时提供 CLI、Web UI 和 QQ Bot 入口。
...
用户消息
   ↓
Session / ContextBuilder 组装上下文
   ↓
Agent Loop 调用模型并决定是否使用工具
   ↓
审批门 / ToolRegistry / Workspace 安全边界
   ↓
工具结果回写 Session，继续推理或返回最终答案
```

项目的核心原则是：

- 所有入口最终汇聚到 `run_agent_turn()`，避免 CLI、Web、QQ 和定时任务各自实现一套 Agent 逻辑。
- `ContextBuilder` 统一组装系统提示词、Soul、历史、记忆、Skill 和运行时信息。
- 文件写入、Shell 命令和 Skill 加载均有明确的安全级别与审批路径。
- 时间相关功能自动识别系统时区，也支持 `CLAW_TIMEZONE` 显式覆盖，识别失败时使用上海时区。

## 界面预览

![SJTUClaw Web UI 首页](docs/images/readme-webui.png)

Web UI 提供会话侧栏、实时对话、工具调用状态、附件处理、主题切换和桌面宠物等能力。启动 Gateway 后即可通过浏览器访问。
...
- 一个 OpenAI 兼容的模型服务，例如 OpenAI、vLLM、Ollama 或 LM Studio
...
如果使用本地模型，可将 `LLM_BASE_URL` 指向 Ollama、vLLM 或 LM Studio 的 OpenAI 兼容接口。系统环境变量优先于 `.env` 文件；运行时数据保存在 `data/`，密钥不应提交到版本库。
...
cd webui
npm install
npm run dev       # 前端开发服务器，默认 http://127.0.0.1:5173
npm run build     # 构建到项目根目录 web/
```

### 第一次使用

1. 启动 `sjtuclaw gateway`，打开 `http://127.0.0.1:8000`。
2. 点击“新对话”，直接输入问题或任务。
3. 需要读写本地文件时，先设置 workspace；写文件和执行 Shell 命令会经过审批。
4. 在侧栏切换、重命名或删除 Session；长期信息可以通过自然语言交给 `remember` 工具保存。
5. 需要自动执行的任务可以直接说“每天早上 9 点提醒我……”，Agent 会调用 Cron 工具创建任务。

## 特色功能

### 统一 Agent Loop

通过 Think → Act → Observe 循环处理普通回答、工具调用、审批、Skill 加载和错误恢复。CLI、Web UI、QQ Bot、Heartbeat 与 Cron 任务共享同一套运行时。

### 安全可控的工具调用

工具按 `read-only`、`write`、`shell` 和 `skill_select` 分级。只读工具可以直接执行，写文件、Shell 和 Skill 加载根据策略进入审批流程；workspace 边界、路径校验、SSRF 防护、请求限流和上传大小限制共同保护本地环境。

### 长期记忆与每日反思

记忆以 Markdown + YAML frontmatter 存储，支持类别、标签、重要性、版本追踪和检索。Reflection 会定期回顾近期对话，自动整理值得保留的信息。

### 可组合的 Skill 系统

Skill 以 `skills/<name>/SKILL.md` 组织，可附带 `assets/` 和 `references/`。Agent 可以发现、查看、按需加载或在审批后创建新的 Skill，适合沉淀课程报告、材料总结、演示大纲等重复工作流。

### 多入口与实时反馈

Web UI 通过 SSE 展示思考事件、工具调用开始/结束和最终回复；CLI 适合本地快速操作，QQ Bot 支持私聊、群聊和内联审批按钮。

### 自动时区与本地化定时任务

当前时间、上下文运行时信息、Cron 默认时区和反思时间均自动跟随系统时区；可使用 `CLAW_TIMEZONE=America/New_York` 等 IANA 时区显式覆盖，无法识别时回退到 `Asia/Shanghai`。
...
│
├── docs/images/                        # README 与项目文档配图
│   └── readme-webui.png               # Web UI 首页截图
│
├── skills/                            # Skill 数据目录
...
9. **时区识别**：用户可见时间和定时任务默认自动跟随系统时区；可用 `CLAW_TIMEZONE=Asia/Shanghai` 等 IANA 时区显式覆盖，识别失败时回退上海时区。
