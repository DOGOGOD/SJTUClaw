...
下载入口不按时间过期，最多保留 1000 条。注册表位于
`data/downloads/registry.json`，因此源文件仍存在的链接可跨时间和 Gateway 重启
继续使用。注册表只保存下载 ID、源文件绝对路径和创建时间，不复制文件内容；
删除或移动源文件，或条目因超过容量上限被淘汰，都会使入口失效。`/downloads/{id}` 只能访问已登记且格式合法的
