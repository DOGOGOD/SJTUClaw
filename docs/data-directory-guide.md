...
使用 Claude Code 后端时，Claude Code 的原生 transcript 和登录配置仍由 Claude Code
保存在其用户目录中。SJTUClaw 只会在 `data/claude/runtime/` 短暂写入当前回合追加的
system prompt、本地审批桥设置、SJTUClaw 独有工具清单和临时 MCP 配置，并在回合
结束后删除。临时 MCP 配置只合并一个本地 stdio 服务，不覆盖用户原有的 Claude Code
MCP。Claude session UUID、generation、owner 和工作目录映射保存在对应的
`data/sessions/*.jsonl` metadata 中。危险工具等待审批或 MCP 工具返回结果时，启动
  ...
  "entries": [
    {
      ...
      "path": "C:\\workspace\\reports\\result.pdf",
      "createdAt": 1785290400.0
    }
  ]
}
```
...
### 11.5 `state.db-shm` 和 `state.db-wal`
...
| `data/claude/runtime/` | 只丢失当前 Claude Code 回合的临时交换文件；原生 Claude 会话和登录配置不在此目录 |
| `data/downloads/` | 已创建的 Web UI 下载链接全部失效，但不会删除 workspace 原文件 |
